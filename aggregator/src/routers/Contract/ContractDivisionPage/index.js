...
class ContractDivisionPage extends Component {
    ...
    pageJumpNext =()=>{
        if (this.nextIsDisiable){
            ...
            let form = store.getState().contract.condition.form;
            form.pageIndex = form.pageIndex + 1;
            store.dispatch(ContractSaveSearchConditionAction(form));
    ...
    pageJump = (op) =>{
        return (e) => {
            let contract = store.getState().contract;
            let form = contract.condition.form;
            let pageCount = contract.table.pageCount;
    ...
    render() {
        const contractState = store.getState().contract;
        const pageIndex = contractState.table.pageCount == 0?0 : contractState.condition.form.pageIndex;
        const pageCount = contractState.table.pageCount;
        let options = [];
        ...
        return (
            <div className={'contractDivisionPage'}>
...
export default ContractDivisionPage;