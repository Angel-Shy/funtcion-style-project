...
import stl from './ElectricContractDetail.module.scss';
import app from '../../../../app.json';
import {subscribeTransactionRequest} from "../../../../axios/transactionRequests";
import {
    NotifyCloseLoading,
    NotifyElectricityTransactionSuccess,
    NotifyMessage,
    NotifyOpenLoading
} from "../../../../pubsub";

function ElectricContractDetail(props) {
    ...
    return (
        <div className={stl.electricContractContainer}>
            ...
            <div className={stl.content}>
                ...
                <code>{element.cmdId}</code> 预计开始执行时间为：<code>{element.directionStartTime}</code>，预计结束时间为： <code>{element.directionEndTime}</code>。在指令执行期间甲方需要按照上级要求减少一定的电量使用，由于甲方自身不具备
                完全承担本次调控下发给甲方的电量任务！所以甲方委托乙方承担部分电量任务(见合同下方)，并向乙方支付一定量的积分 (见合同下方)！合同完成双方签署后，甲方完成对乙方的积分支付，乙方有责任承担合同规定的原本由甲方承担的电量任务，
                并且该部分电量对应的系统奖励积分在乙方完成该部分电量调控任务后，由乙方获得！如果乙方未完成该部分电力任务，则责任由乙方自行承担！
            </div>
            <div className={stl.tfooter}>
                <p >
                    <span className={stl.leftItem}>甲方支付积分:</span>
                    <span className={stl.itemAnswer}>{element.point}</span>
                </p>
                <p>
                    <span className={stl.leftItem}>乙方承担电量:</span>
