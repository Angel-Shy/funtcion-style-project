...
import stl from "../MyHotTradeElectricTable/MyHotTradeElectricTable.module.scss";
import {
    NotifyMessage,
...
import RefuseReasonDetail from "./RefuseReasonDetail/RefuseReasonDetail";
...
function PublishTradeElectricTable() {
    ...
    /* 2022/7/23 检索select */
    React.useEffect( () => {
        getOthersRequirement({type:1},(data) => {
            ...
            options.unshift( {name:'全部电力交易需求', value: -1});
    ...
    let jumpGetDataFromServer = useCallback(op => {
        ...
        getPublishTransactionRequest(condition, data => {
            let action = {
                        ...
                        store.tableData.data.map( (element,idx) => {
                            return (
                                <tr key={element.transactionId}>
                                    ...
                                    <td> {element.state === 1?"等待确认": element.state===2?"已取消":"已拒绝"}</td>
                                    <td>
                                        {
                                            element.state === 1?<button className={stl.linkDetailOperation} onClick={cancelTransactionEvent(element.transactionId)}>取消交易</button>:""
                                        }
                                        {
                                            element.state === 2?<span className={'font-blue padding-right-10px'} > 无法操作 </span> :''
                                        }
                                        {
            ...
            </div>
            {
                store.noticeIsShow?
                    <DialogContainer
                        ...
                        render={ <RefuseReasonDetail reason={store.reason} close={() => {dispatch({type: CloseNotice});}}/> }
...
export default PublishTradeElectricTable;
